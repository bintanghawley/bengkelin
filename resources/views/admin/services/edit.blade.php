...
<div class="flex min-h-screen font-sans">
    ...
    <main class="flex-1 ml-64 p-10 bg-zinc-950 min-h-screen">
        <div class="max-w-3xl">
            <div class="mb-8">
                <h1 class="text-2xl font-bengkel uppercase tracking-widest text-white">Edit Layanan Servis</h1>
                <p class="text-zinc-500 text-xs mt-1 uppercase tracking-widest">Perbarui detail: <span class="text-red-500">{{ $service->nama }}</span></p>
            </div>
            ...
            <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-zinc-900 rounded-2xl border border-zinc-800 p-6 space-y-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $service->nama) }}" required
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="deskripsi" rows="3" required
                                  class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-red-600 outline-none transition resize-none">{{ old('deskripsi', $service->deskripsi) }}</textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            <label class="text-[10px] uppercase text-zinc-500 font-bold">Harga Mulai (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" name="harga_mulai" value="{{ old('harga_mulai', $service->harga_mulai) }}" min="0" required
                                   class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-red-600 outline-none transition">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] uppercase text-zinc-500 font-bold">Estimasi Waktu <span class="text-red-500">*</span></label>
                            <input type="text" name="estimasi_waktu" value="{{ old('estimasi_waktu', $service->estimasi_waktu) }}" required
                                   class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-red-600 outline-none transition">
                        </div>
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] uppercase text-zinc-500 font-bold">Gambar Servis</label>
                        @if($service->gambar)
                            <div class="flex items-center gap-4 bg-zinc-950 border border-zinc-800 rounded-xl p-3">
                                <img src="{{ asset('storage/' . $service->gambar) }}" alt="{{ $service->nama }}"
                                     class="w-16 h-16 object-cover rounded-lg border border-zinc-700">
                                <div class="text-xs text-zinc-500">Gambar saat ini. Upload baru untuk mengganti.</div>
                            </div>
                        @endif
                        ...
                        <p class="text-[10px] text-zinc-600">Format: JPG, PNG, WEBP. Maks 2MB. Kosongkan jika tidak ingin mengubah.</p>
                    </div>
                </div>
                    ...
                    <div id="items-container" class="space-y-3">
                        ...
                        <div class="item-row flex items-center gap-3">
                            <div class="flex-1 flex items-center gap-3 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3">
                                ...
                                <input type="text" name="items[]" value="{{ $item->nama_pekerjaan }}"

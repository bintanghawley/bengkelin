...
<div class="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-300">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex items-center gap-2 text-xs text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-8">
            ...
            <span>/</span>
            <a href="{{ route('toko.sparepart.show', $sparepart->id) }}" class="hover:text-blue-600 transition">{{ $sparepart->nama }}</a>
            <span>/</span>
        ...
        <h1 class="text-3xl font-bengkel text-zinc-900 dark:text-white tracking-wider uppercase mb-8">
            Halaman <span class="text-blue-600">Checkout Sparepart</span>
        </h1>
        ...
        <form action="{{ route('toko.sparepart.buy', $sparepart->id) }}" method="POST" id="checkout-form">
            @csrf
                ...
                <div class="lg:col-span-7 space-y-6">
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="telepon" id="telepon" 
                                ...
                                class="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 text-zinc-800 dark:text-white rounded-xl text-sm border border-zinc-200 dark:border-zinc-750 focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600 transition"
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="catatan" id="catatan" rows="2"
                                class="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 text-zinc-800 dark:text-white rounded-xl text-sm border border-zinc-200 dark:border-zinc-750 focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600 transition"
                                placeholder="Contoh: Titipkan di satpam, minta dipacking kayu, dll.">{{ old('catatan') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-6 md:p-8 shadow-sm space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                            {{-- COD Option --}}
                            <label class="relative border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 flex items-center gap-4 cursor-pointer hover:border-blue-600 transition group">
                                <input type="radio" name="metode_pembayaran" value="COD" checked
                                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-zinc-300 dark:border-zinc-750">
                                <div>
                                    <span class="block font-bold text-sm uppercase tracking-wide">Cash on Delivery (COD)</span>
                                    <span class="block text-zinc-400 dark:text-zinc-500 text-[10px] mt-0.5">Bayar tunai saat sparepart sampai</span>
                                </div>
                            </label>
                    ...
                    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-6 md:p-8 shadow-sm space-y-6 sticky top-6">
                        ...
                        <div class="flex gap-4 items-start pb-6 border-b border-zinc-100 dark:border-zinc-800">
                            <div class="w-20 h-20 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-750 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center">
                                @if($sparepart->gambar)
                                    <img src="{{ asset('storage/' . $sparepart->gambar) }}" alt="{{ $sparepart->nama }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <div class="flex-1 space-y-1">
                                <span class="inline-block px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-600 dark:text-blue-400">
                                    {{ $sparepart->jenis_sparepart }} | {{ $sparepart->merek }}
                                </span>
                                <h3 class="font-bengkel uppercase text-sm tracking-wide text-zinc-900 dark:text-white line-clamp-2">
                                    {{ $sparepart->nama }}
                                </h3>
                                <p class="text-xs text-zinc-400 dark:text-zinc-500">
                                    Stok: {{ $sparepart->stok }} pcs
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center py-4 border-b border-zinc-100 dark:border-zinc-800">
                            ...
                            <div class="flex items-center gap-1.5 bg-zinc-50 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-750 p-1.5 rounded-2xl">
                                <button type="button" onclick="adjustQty(-1)" class="w-8 h-8 flex items-center justify-center bg-white dark:bg-zinc-700 hover:bg-blue-500 hover:text-white border border-zinc-200 dark:border-zinc-650 rounded-xl transition text-sm font-bold shadow-sm">
                                    -
                                </button>
                                <input type="number" name="jumlah" id="jumlah" min="1" max="{{ $sparepart->stok }}" value="1" readonly
                        ...
                        <div class="space-y-3 pt-2 text-xs">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">Rp {{ number_format($sparepart->harga, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-zinc-500 dark:text-zinc-400">Subtotal Barang</span>
                                <span class="font-medium" id="subtotal-val">Rp {{ number_format($sparepart->harga, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="pt-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-baseline">
                                ...
                                <span class="text-2xl font-bengkel text-blue-600" id="total-val">
                        ...
                        <div class="space-y-3 pt-4">
                            <button type="submit" class="w-full px-6 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl text-sm font-bold uppercase tracking-widest transition shadow-lg shadow-blue-600/20">
                                Konfirmasi & Bayar
                            </button>
                            <a href="{{ route('toko.sparepart.show', $sparepart->id) }}" class="block w-full text-center px-6 py-3 bg-zinc-100 dark:bg-zinc-800/80 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-2xl text-xs font-bold uppercase tracking-widest border border-zinc-200 dark:border-zinc-750 transition">
                                Batal
                            </a>
                        </div>
                    </div>
...
<script>
    const itemPrice = {{ $sparepart->harga }};
    const maxQty = {{ $sparepart->stok }};
